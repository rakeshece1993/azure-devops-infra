...
resource "azurerm_kubernetes_cluster" "aks_cluster" {
  ...
  dns_prefix                          = var.prefix #check for this in main.tf local vars.
  ...
  private_cluster_enabled             = var.private_cluster_enabled #Cluste will be create in privare if true, current is false
  ...
  dynamic "default_node_pool" {
    ...
    content {
      ...
      tags                         = merge(var.resource_tags, var.agents_tags)
  ...
  depends_on = [
    null_resource.kubernetes_cluster_name_keeper,
  ]
...
resource "azapi_update_resource" "aks_cluster_post_create" {
  ...
  depends_on = [
    ...
    azurerm_kubernetes_cluster_node_pool.aks_node_pool
  ]
...
resource "azapi_update_resource" "aks_cluster_default_nodepool_version" {
  ...
  resource_id = "${azurerm_kubernetes_cluster.aks_cluster.id}/agentPools/${var.system_pool_name}"
...
resource "azurerm_kubernetes_cluster_node_pool" "aks_node_pool" {
  for_each              = var.user_node_pool_details
